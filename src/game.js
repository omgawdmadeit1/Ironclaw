...
import { Level, ParticleSystem } from "./level.js";
...
class Input {
  ...
  down(action, playerId = "p1") {
    ...
    return map.some((k) => this.keys.has(k));
  }

  pressed(action, playerId = "p1") {
    const map = this.maps[playerId]?.[action] || this.maps.system[action] || [];
    return map.some((k) => this.justPressed.has(k) || (this.keys.has(k) && !this.prev.has(k)));
  }
...
export class Game {
  ...
  menuAction(action) {
    ...
    if (action === "play") this.newRun();
    if (action === "resume") this.mode = "play";
    if (action === "restartStage") this.restartStage();
    if (action === "musicDown") this.audio.setMusicVolume(this.audio.musicVolume - 0.1);
    if (action === "musicUp") this.audio.setMusicVolume(this.audio.musicVolume + 0.1);
    if (action === "sfxDown") this.audio.setSfxVolume(this.audio.sfxVolume - 0.1);
    if (action === "sfxUp") this.audio.setSfxVolume(this.audio.sfxVolume + 0.1);
    if (action === "toggleMute") this.audio.toggleMute();
    this.ui.render();
  }
  ...
  newRun() {
    ...
    this.players = [
      ...
      this.createPlayer(1, FIGHTERS[this.selectedFighters[1] ?? 1], 165, this.level.bounds.top + 150)
    ];
  ...
  update(dt) {
    ...
    if (this.leadingPlayerX() > this.level.length - 120 && this.enemies.length === 0) this.nextStage();
  }
  ...
  spawnWaves() {
    const leadX = this.leadingPlayerX();
    for (let i = 0; i < this.level.waves.length; i++) {
      const wave = this.level.waves[i];
      if (this.level.spawned.has(i) || leadX < wave.x) continue;
      this.level.spawned.add(i);
      if (wave.boss) {
        this.enemies.push(this.scaleEnemyForCoop(new Boss(leadX + 360, this.level.bounds.top + 88)));
        this.audio.sfx("boss");
        this.bossWarningTimer = 2.2;
      } else {
        for (const [type, x, y] of wave.enemies) this.enemies.push(this.scaleEnemyForCoop(new Enemy(type, x, y)));
      }
    }
  }
  ...
  resolvePickups() {
    for (const pickup of this.level.pickups) {
      ...
      const candidates = this.activePlayers()
        .filter((player) => Math.hypot(pickup.x - player.x, pickup.y - player.y) <= 84)
      ...
      for (const player of candidates) {
        ...
        if (pickup.type === "gem") player.score += 250;
