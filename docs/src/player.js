...
export class Player extends Entity {
  ...
  update(dt, input, level, audio, game = null) {
    ...
    if (this.dashTimer > 0 && this.upgrades.dashInvuln) this.invuln = Math.max(this.invuln, 0.08);
  ...
  equipWeapon(id) {
    ...
    if (this.upgrades.weaponDurability) {
      this.weapon.maxUses += this.upgrades.weaponDurability;
      this.weapon.uses += this.upgrades.weaponDurability;
    }
    this.weaponSwingId = -1;
  }
  ...
  attackSpec() {
    ...
    const comboBoost = 1 + (this.combo >= 3 ? this.upgrades.comboDamage : 0);
    const crit = this.upgrades.critChance > 0 && Math.random() < this.upgrades.critChance;
    ...
    if (weaponKind && this.weapon) {
      ...
      return {
        ...base,
        damage: Math.round(base.damage * this.power * boost * powerBoost * comboBoost * (crit ? 1.45 : 1)),
    ...
    return spec && {
      ...spec,
      damage: Math.round(spec.damage * this.power * boost * comboBoost * (crit ? 1.45 : 1)),
